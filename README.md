...
```
Определите переменную окружения `YANDEX_GEOCODER_KEY`.
```sh
YANDEX_GEOCODER_KEY='YOUR_YANDEX_GEOCODER_KEY'
```
В значение данной переменной поместите значение ключа от API геокодера "Яндекс".
Инструкция по получению: см. первые шаги по ссылке https://dvmn.org/encyclopedia/api-docs/yandex-geocoder-api/
